...
Greenspot.CartStallItem = function (stallId, stallName) {
    ...
    self.add = function (nwItem) {
        ...
        if (!itemAdded) {
            //new item
            self.itms.push({ i: nwItem.id, n: nwItem.name, v: nwItem.variant, q: 1, p: nwItem.price, slctd: true });
        }
    };
...
Greenspot.Cart = function () {
    ...
    self.getOrders = function () {
        var orders = new Greenspot.OrderCollection();
        $.each(self.stls, function (idxStall, stall) {
            //new order
            var order = new Greenspot.Order(stall.i, stall.n);
...
Greenspot.Order = function (stallId, stallName) {
    var self = this;
    self.i = stallId;
    self.n = stallName;
    self.qty = 0;
...
Greenspot.OrderCollection = function () {
    var self = this;
    self.orders = [];
    self.amt = 0;
